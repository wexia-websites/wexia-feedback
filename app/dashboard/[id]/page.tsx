...
import {
  ...
  CATEGORIES, STATUSES, STATUS_ORDER, STATUS_ICON, CAT_ICON,
  fmtDate, timeAgo, deriveTitle,
  type CategoryId, type StatusId,
} from '@/lib/feedback-ui'

type Note = { author: string; at: string; text: string }
        ...
        <>
          <div style={{ marginBottom: 14 }}>
            <div style={{
              ...
              background: 'var(--bg-2)', borderRadius: '8px 8px 0 0',
              padding: '6px 12px', borderBottom: '1px solid var(--border)',
            }}>
              <span style={{ color: 'var(--text-3)', fontSize: 11, fontFamily: 'monospace' }}>HTML</span>
              <button
                onClick={copyHtml}
                className={copied ? 'btn btn-success' : 'btn btn-ghost'}
            ...
            <pre style={{
              ...
              padding: 14, maxHeight: 150, overflowY: 'auto', overflowX: 'auto', margin: 0,
            }}>
              <code style={{ fontSize: 12, color: 'var(--text-2)', fontFamily: 'monospace', whiteSpace: 'pre-wrap', wordBreak: 'break-word' }}>
                {html}
              </code>
            </pre>
          </div>
...
export default function FeedbackDetailPage() {
  ...
  async function addNote() {
    ...
    setAddingNote(true)
    const newNote: Note = { author: 'Tým', at: new Date().toISOString(), text: noteDraft.trim() }
    ...
    const { error } = await supabase
      ...
      .eq('id', item.id)
    if (!error) {
      setItem(prev => prev ? { ...prev, notes } : prev)
      setNoteDraft('')
    }
    setAddingNote(false)
  }
  ...
  // Prev / Next navigation (sorted by timestamp desc — prev = newer, next = older)
    ...
    <>
      ...
      <div style={{ padding: '18px 32px 0', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <button
          ...
          style={{
            ...
            padding: '8px 12px 8px 8px', borderRadius: 9,
        ...
        <div style={{
          ...
          <button
            disabled={!prevId}
            onClick={() => prevId && router.push(`/dashboard/${prevId}`)}
            title="Novější"
            style={{ width: 28, height: 28, borderRadius: 7, display: 'grid', placeItems: 'center', color: prevId ? 'var(--text-2)' : 'var(--border-2)', background: 'none', border: 'none', cursor: prevId ? 'pointer' : 'not-allowed', transition: 'background 0.14s' }}
          ...
          <button
            disabled={!nextId}
            onClick={() => nextId && router.push(`/dashboard/${nextId}`)}
            title="Starší"
            style={{ width: 28, height: 28, borderRadius: 7, display: 'grid', placeItems: 'center', color: nextId ? 'var(--text-2)' : 'var(--border-2)', background: 'none', border: 'none', cursor: nextId ? 'pointer' : 'not-allowed', transition: 'background 0.14s' }}
      ...
      <div style={{ padding: '20px 32px 60px', display: 'grid', gridTemplateColumns: '1fr 332px', gap: 20, alignItems: 'start' }}>
        ...
        <div>
          ...
          <h1 style={{ fontSize: 25, fontWeight: 800, letterSpacing: '-0.025em', lineHeight: 1.2, marginBottom: 20, color: 'var(--text)' }}>
            {title}
          </h1>
          ...
          <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 14, padding: '18px 20px', marginBottom: 20 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 12.5, fontWeight: 700, color: 'var(--text-3)', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 11 }}>
              <Icon name="comment" size={15} style={{ color: 'var(--accent-hi)' }} />
              Komentář uživatele
            </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '2px 26px', background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 14, padding: '8px 20px', marginBottom: 20 }}>
            {[
              { icon: 'mail',     label: 'E-mail',     value: item.user_email || '—', mono: true },
              { icon: 'globe',    label: 'Stránka',    value: item.url || '—',        mono: true, link: item.url },
              { icon: 'monitor',  label: 'Prohlížeč',  value: item.user_agent || '—', mono: false },
              { icon: 'calendar', label: 'Přijato',    value: fmtDate(item.timestamp), mono: false },
            ].map((m, i, arr) => (
              <div key={m.label} style={{
                ...
                <div style={{ minWidth: 0 }}>
                  ...
                  {m.link ? (
                    <a href={m.link} target="_blank" rel="noopener noreferrer" style={{ fontSize: 13.5, fontWeight: 600, marginTop: 2, display: 'block', color: 'var(--accent-hi)', fontFamily: m.mono ? 'monospace' : 'inherit', wordBreak: 'break-all' }}>
                      {m.value}
                    </a>
                  ) : (
                    <div style={{ fontSize: 13.5, fontWeight: 600, marginTop: 2, color: 'var(--text)', fontFamily: m.mono ? 'monospace' : 'inherit', wordBreak: 'break-all' }}>
                      {m.value}
                    </div>
                  )}
                </div>
        ...
        <aside style={{ display: 'flex', flexDirection: 'column', gap: 14, position: 'sticky', top: 20 }}>
          ...
          <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 14, padding: 16, boxShadow: 'var(--shadow)' }}>
            ...
            {successMsg && (
              <div className="success-banner" style={{ marginBottom: 10 }}>✓ {successMsg}</div>
            )}
                ...
                return (
                  <button
                    ...
                    style={{
                      ...
                      padding: '10px 11px', borderRadius: 10, fontSize: 13, fontWeight: 700,
                      fontFamily: 'inherit', cursor: isOn ? 'default' : 'pointer',
                      color: isOn
                        ? (st.muted ? 'var(--text)' : `oklch(0.86 0.11 ${st.hue})`)
                        : 'var(--text-2)',
                      background: isOn
                        ? (st.muted ? 'var(--surface-hi)' : `oklch(0.7 0.13 ${st.hue} / 0.14)`)
                        : 'var(--surface-2)',
                      border: isOn
                        ? (st.muted ? '1px solid var(--border-2)' : `1px solid oklch(0.7 0.13 ${st.hue} / 0.4)`)
          ...
          <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 14, padding: 16, boxShadow: 'var(--shadow)' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12, marginBottom: notes.length > 0 ? 13 : 0 }}>
              {notes.length === 0 && (
                <div style={{ fontSize: 13, color: 'var(--text-3)', fontStyle: 'italic', paddingBottom: 4 }}>
            ...
            <div>
              <textarea
                ...
                placeholder="Přidat poznámku k řešení… (Ctrl+Enter)"
                style={{
                  width: '100%', minHeight: 64, resize: 'vertical',
                  background: 'var(--surface-2)', border: '1px solid var(--border)',
                  borderRadius: 11, padding: '11px 13px',
                  color: 'var(--text)', fontFamily: 'inherit', fontSize: 13.5, lineHeight: 1.5,
              ...
              <button
                ...
                style={{
                  ...
                  width: '100%', marginTop: 9, padding: 11, borderRadius: 10,
                  fontSize: 13.5, fontWeight: 700, color: '#fff',
                  background: 'var(--accent)', border: 'none', cursor: 'pointer',
                  fontFamily: 'inherit', transition: 'background 0.14s, opacity 0.14s',
                  opacity: !noteDraft.trim() || addingNote ? 0.45 : 1,
                }}
